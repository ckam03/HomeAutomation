...
async def main():
    ...
    command = voice_input.get_audio()
    print(command)
    response = await get_llm_response(command)
    print(response)

    # await tools.toggle_desk_light_on("on")
    # get the audio here
    # use gemini to use this text to pick the correct tool
